...
def MDN_output_layer(x, h, y, in_size, out_size, hidden_size, pred):
    ...
    mu_linear = Linear(name='mu_linear' + str(pred), input_dim=hidden_out_size,
                       output_dim=out_size * components_size[network_mode])
    sigma_linear = Linear(name='sigma_linear' + str(pred), input_dim=hidden_out_size,
                          output_dim=components_size[network_mode])
    mixing_linear = Linear(name='mixing_linear' + str(pred), input_dim=hidden_out_size,
                           output_dim=components_size[network_mode])
...
def rnn_layer(in_size, dim, x, h, n, first_layer=False):
    if connect_h_to_h == 'all-previous':
        ...
        if first_layer:
            ...
            linear = Linear(input_dim=in_size + dim * 2 if n > 1 else in_size + dim, output_dim=dim,
                            name='linear' + str(n) + '-')
        else:
    ...
    if layer_models[n] == 'rnn':
        ...
        return rnn.apply(linear.apply(rnn_input), time_scale=layer_resolutions[n],
...
def lstm_layer(in_size, dim, x, h, n, first_layer=False):
    if connect_h_to_h == 'all-previous':
        ...
        if first_layer:
            ...
            linear = Linear(input_dim=in_size + dim * 2 if n > 1 else in_size + dim, output_dim=dim * 4,
                            name='linear' + str(n) + '-')
        else:
    ...
    if layer_models[network_mode][n] == 'lstm':
        ...
        return lstm.apply(linear.apply(lstm_input), time_scale=layer_resolutions[n],
                          time_offset=layer_execution_time_offset[n])
